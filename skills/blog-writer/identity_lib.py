"""Shared validation and path contracts for writing-identity tools."""
...
CURRENT_SELECTION_SCHEMA = 2
SELECTION_SCHEMAS = {1, CURRENT_SELECTION_SCHEMA}
...
class ToolError(RuntimeError):
    """An identity tool could not inspect or update the requested files."""
...
def resolve_relative(raw: str, base: Path, *, allow_named_user: bool = False) -> Path:
    try:
        if uses_named_user_path(raw) and not allow_named_user:
            raise IdentityError(
...
def validate_identity(
    ...
    base: Path,
    *,
    allow_named_user: bool = False,
) -> dict:
    root = resolve_relative(raw_path, base, allow_named_user=allow_named_user)
...
def load_selection_config(path: Path) -> dict:
    config = load_json(path)
    errors: list[str] = []
    schema_version = config.get("schema_version")
    if schema_version not in SELECTION_SCHEMAS:
    ...
    for key in ("personal", "corporate"):
        if key in config and (not isinstance(config[key], str) or not config[key]):
            ...
            schema_version == CURRENT_SELECTION_SCHEMA
            and key in config
            and uses_named_user_path(config[key])
        ):
